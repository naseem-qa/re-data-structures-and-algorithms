...
const listFoods = (recipe) => {
    ...
    recipe.ingredients.forEach(element => {
        const removAmount = element.slice(element.indexOf(' ') + 1);
        const removUnit = removAmount.slice(removAmount.indexOf(' ') + 1);
        result.push(removUnit);
    });
    return result;
};
...
const stepActions = (recipe) => {
    ...
    recipe.steps.forEach(element => {
        const removeEnd = element.split(' ');
        const gitaAtion = removeEnd[0];
        result.push(gitaAtion);
    });
    return result;
};
